        ...
        for idx in range(order.size):
            ...
            try:
                ...
                if img.shape[1] != resolution:
                    raise Exception('Input images must have the same width and height')
                if resolution != 2 ** int(np.floor(np.log2(resolution))):
                    raise Exception('Input image resolution must be a power-of-two')
                if channels != 3:
                ...
                print(str(e) + " for image " + image_filenames[order[idx]] + ", skipping...")
